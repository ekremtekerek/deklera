...
/**
 * Deklera'da "dil" tek bir şey değildir; üç ayrı eksendir:
 *
...
final class Locale {
	 ...
	 */
	public const META_OVERRIDE = '_deklera_document_locale';

	/**
	 * Sipariş oluşturulduğu anda yakalanan locale.
	 */
	public const META_CAPTURED = '_deklera_checkout_locale';
	...
	public static function regulatory_profile(): string {
		 ...
		 */
		return (string) \apply_filters( 'deklera/regulatory_profile', $country );
	}
	...
	private static function ensure_plugin_translations( string $locale ): void {
		$seen = \get_site_transient( 'deklera_plugin_translations' );
		...
		$seen[ $locale ] = \time();
		\set_site_transient( 'deklera_plugin_translations', $seen, \DAY_IN_SECONDS );
	...
	private static function install_language_pack( string $locale ): bool {
		$failed = \get_site_transient( 'deklera_locale_install_failed' );
		...
		$failed[ $locale ] = \time();
		\set_site_transient( 'deklera_locale_install_failed', $failed, \DAY_IN_SECONDS );
		 ...
		 */
		\do_action( 'deklera/language_pack_failed', $locale );

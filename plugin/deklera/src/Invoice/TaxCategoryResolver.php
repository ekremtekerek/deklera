	...
	public static function explain( string $reason ): string {
		switch ( $reason ) {
			case 'seller_outside_eu':
				return __( 'The store is based outside the EU, so the supply is outside the scope of EU VAT.', 'deklera' );

			case 'domestic_rated':
				return __( 'Domestic sale with a VAT rate applied.', 'deklera' );

			case 'oss_distance_selling':
				return __( 'Cross-border sale to an EU consumer, taxed at the destination rate under OSS.', 'deklera' );

			case 'export_outside_eu':
				return __( 'The buyer is outside the EU, so the supply is treated as an export.', 'deklera' );

			case 'intra_community_service':
				return __( 'Assumed to be a service because every item is virtual or downloadable.', 'deklera' );

			case 'intra_community_goods':
				return __( 'Assumed to be goods because the order contains shippable items.', 'deklera' );

			case 'cross_border_zero_rate_suspicious':
				return __( 'Cross-border EU sale with no VAT and no valid buyer VAT number.', 'deklera' );

			case 'domestic_zero_rate':
				return __( 'Domestic sale with no VAT applied.', 'deklera' );

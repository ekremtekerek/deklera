...
final class ExemptionReason {
	...
	public static function text( string $category ): string {
		$text = match ( $category ) {
			'AE'    => __( 'Reverse charge: VAT is due from the recipient.', 'deklera' ),
			'K'     => __( 'Intra-Community supply, exempt under Article 138 of Council Directive 2006/112/EC.', 'deklera' ),
			'G'     => __( 'Export outside the European Union, exempt from VAT.', 'deklera' ),
			'E'     => __( 'Exempt from VAT.', 'deklera' ),
			'O'     => __( 'Not subject to VAT.', 'deklera' ),
			default => '',
		};
		 ...
		 * @param string $category Vergi kategorisi kodu.
		 */
		return (string) \apply_filters( 'deklera/exemption_reason', $text, $category );
	}

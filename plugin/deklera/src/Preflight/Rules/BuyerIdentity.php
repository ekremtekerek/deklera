...
final class BuyerIdentity implements OrderRule {
	...
	public function title(): string {
		return __( 'Customer identity', 'deklera' );
	}
	...
	public function check( SemanticInvoice $invoice, \WC_Order $order ): array {
		...
		if ( '' === trim( $buyer->name ) ) {
			$findings[] = new Finding(
				...
				Severity::BLOCKER,
				$id,
				__( 'The order has no customer name.', 'deklera' ),
				__( 'The buyer name is mandatory on every invoice.', 'deklera' ),
				__( 'Open the order and fill in the billing name or company.', 'deklera' ),
				'BT-44 / BR-07'
			);
		}
		...
		if ( '' === trim( $buyer->country ) ) {
			$findings[] = new Finding(
				...
				Severity::BLOCKER,
				$id,
				__( 'The order has no billing country.', 'deklera' ),
				__( 'The buyer country determines the VAT treatment. Without it the tax category cannot be decided.', 'deklera' ),
				__( 'Open the order and set the billing country.', 'deklera' ),
				'BT-55 / BR-11'
			);
		...
		if ( '' === trim( $buyer->vat_number ) ) {
			$findings[] = new Finding(
				...
				Severity::BLOCKER,
				$id,
				__( 'This is a cross-border EU business sale with no VAT, but the customer VAT number is missing.', 'deklera' ),
				__( 'When VAT is not charged on an intra-community supply, the buyer VAT identifier is mandatory. Without it the exemption cannot be justified and the invoice is rejected.', 'deklera' ),
				__( 'Open the order and add the VAT number to the billing details, then start collecting it at checkout.', 'deklera' ),
				'BT-48 / BR-AE-09'
			);
		...
		if ( ! Eu::looks_like_vat_number( $buyer->vat_number ) ) {
			$findings[] = new Finding(
				...
				sprintf(
					...
					__( 'The customer VAT number "%s" is not in a valid EU format.', 'deklera' ),
					$buyer->vat_number
				),
				__( 'An EU VAT number starts with the two-letter country code, for example DE123456789.', 'deklera' ),
				__( 'Open the order and correct the VAT number in the billing details.', 'deklera' ),
				'BT-48'
			);
		...
		if ( '' !== $prefix && strtoupper( $buyer->country ) !== $prefix ) {
			$findings[] = new Finding(
				...
				sprintf(
					...
					__( 'The VAT number is registered in %1$s but the billing country is %2$s.', 'deklera' ),
					$prefix,
					strtoupper( $buyer->country )
				),
				__( 'A mismatch is legitimate for branch offices, but it is also a common sign of a mistyped VAT number.', 'deklera' ),
				__( 'Open the order and confirm both values with the customer.', 'deklera' ),
				'BT-48 / BT-55'
			);
		}

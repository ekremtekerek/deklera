...
final class TotalsConsistency implements OrderRule {
	...
	public function title(): string {
		return __( 'Invoice totals', 'deklera' );
	}
		...
		if ( $drift > self::TOLERANCE ) {
			$findings[] = new Finding(
				...
				sprintf(
					...
					__( 'The invoice lines add up to %1$s but the order total is %2$s, a difference of %3$s.', 'deklera' ),
					$this->money( $mapped, $invoice->currency ),
					$this->money( $actual, $invoice->currency ),
					$this->money( $drift, $invoice->currency )
				),
				__( 'Validators reject an invoice whose totals do not reconcile, even by one cent. This usually comes from an order-level discount, a coupon or a refund that is not represented on any line.', 'deklera' ),
				__( 'Open the order and compare the line totals with the order total.', 'deklera' ),
				'BT-112 / BR-CO-15'
			);
		}
		...
		if ( round( abs( $tax_total - $order_tax ), 2 ) > self::TOLERANCE ) {
			$findings[] = new Finding(
				...
				sprintf(
					...
					__( 'The VAT breakdown totals %1$s but WooCommerce recorded %2$s.', 'deklera' ),
					$this->money( $tax_total, $invoice->currency ),
					$this->money( $order_tax, $invoice->currency )
				),
				__( 'The sum of the VAT breakdown must equal the VAT charged. A gap means a rate was applied that Deklera could not reconstruct from the order.', 'deklera' ),
				__( 'Review the tax lines on the order and the rounding setting under WooCommerce > Settings > Tax.', 'deklera' ),
				'BT-110 / BR-CO-13'
			);
		}

...
final class AuditLog {
	...
	public static function label( string $event ): string {
		return match ( $event ) {
			self::EVENT_GENERATED  => __( 'Document generated', 'deklera' ),
			self::EVENT_FAILED     => __( 'Generation failed', 'deklera' ),
			self::EVENT_DOWNLOADED => __( 'Document downloaded', 'deklera' ),
			self::EVENT_QUEUED     => __( 'Queued for generation', 'deklera' ),
			self::EVENT_INVALID    => __( 'Rejected by official validation', 'deklera' ),
			self::EVENT_EMAILED    => __( 'Attached to customer email', 'deklera' ),
			self::EVENT_PRUNED     => __( 'Removed after the retention period', 'deklera' ),
			self::EVENT_KSEF_SENT       => __( 'Sent to KSeF', 'deklera' ),
			self::EVENT_KSEF_REGISTERED => __( 'Registered by KSeF', 'deklera' ),
			self::EVENT_KSEF_REJECTED   => __( 'Rejected by KSeF', 'deklera' ),
			default                => $event,
		};
	}
}

<?php
/**
 * Lisans zincirini ucdan uca olcer.
 *
 * Kullanim:
 *   docker compose run --rm wpcli wp eval-file \
 *     wp-content/plugins/deklera/tests/plan-probe.php
 *
 * @package Deklera
 */

/*
 * Bu dosya WP-CLI'de "wp eval-file" ile calisir; ciktisi terminale gider,
 * HTML'e degil. Kacis kurallari burada yanlis pozitif uretir. Global onek
 * kurali da oyle: bunlar eval-file kapsamindaki yerel degiskenler, gercek
 * global degil.
 */
// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound

$fs = function_exists( 'deklera_fs' ) ? deklera_fs() : null;

printf( "deklera_fs()            : %s\n", is_object( $fs ) ? get_class( $fs ) : 'yok' );

if ( is_object( $fs ) ) {
	printf( "is_premium (kod)        : %s\n", $fs->is_premium() ? 'true' : 'false' );
	printf( "can_use_premium_code()  : %s\n", $fs->can_use_premium_code() ? 'true' : 'false' );
	printf( "is_registered()         : %s\n", $fs->is_registered() ? 'evet' : 'hayir' );
	printf( "is_paying()             : %s\n", $fs->is_paying() ? 'evet' : 'hayir' );
	printf( "is_trial()              : %s\n", $fs->is_trial() ? 'evet' : 'hayir' );

	$license = $fs->_get_license();
	printf( "lisans                  : %s\n", is_object( $license ) ? 'var (id ' . $license->id . ')' : 'yok' );
}

printf( "Licensing::plan()       : %s\n", Deklera\License\Licensing::plan()->value );
printf( "has_hosted_validation() : %s\n", Deklera\License\Licensing::has_hosted_validation() ? 'ACIK' : 'kapali' );

$validator = new Deklera\Validation\HostedValidator();

printf( "validator yapilandirildi: %s\n", $validator->is_configured() ? 'evet' : 'hayir' );
